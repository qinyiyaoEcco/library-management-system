...
public class ScrollRemoveCommandTest {

    @Autowired
    private ScrollRemoveCommand scrollRemoveCommand;
    ...
    public void testRemoveScroll() {
        String[] args = {"scroll-remove", "-u", "admin", "-p", "admin", "--id", "1"};
        scrollRemoveCommand.run(args);
        Assertions.assertEquals(0, scrollRemoveCommand.getExitCode());
    }
}